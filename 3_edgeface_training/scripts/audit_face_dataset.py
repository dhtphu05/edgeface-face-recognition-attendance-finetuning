...
def main() -> None:
    args = parse_args()
    class_dirs = sorted([path for path in args.dataset_root.iterdir() if path.is_dir()])
    if not class_dirs:
        raise ValueError(f"No identity folders found under: {args.dataset_root}")

    report: dict[str, object] = {"dataset_root": str(args.dataset_root), "classes": {}}
